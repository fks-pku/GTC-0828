...
subroutine gkLoad(species_name,w_initial)
  ...
  use interfaces,only: gkLoadParticle
  implicit none
  ...
  if(species_name=='thermal-ion')then
    ...
    allocate(pmarki(0:mpsi),markeri(mgrid),densityi(0:1,mgrid),&
      flowi(0:1,mgrid),pressureipara(0:1,mgrid),pressureiperp(0:1,mgrid),pfluxi(0:mpsi),rdtemi(0:mpsi),zonali(0:mpsi),&
