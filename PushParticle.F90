...
subroutine pushParticle(zpart,zpart0,wppart,wtpart0,wtpart1,&
  ...
  use global_parameters,only: nonlinear,irk,tstep,magnetic,rg0,eqcurrent,&
    ...
    torbound,rho0,track_particles,nhybrid,numereq,mthetamax,irestore,mstepall
...
! for out-of-boundary particles
  ...
  if(.not. subcycle)then
    ...
    call locateParticle(zpart,wzpart,wppart,wtpart0,wtpart1,jtpart0,&
...
!    er=   erpp(1,isp)   +erpp(2,isp)*dx(2)   +erpp(3,isp)*dx(3)
 ...
 wisland = (-(kappa+kappav)*(upara)*gqi*(dapidz*ri-dapidt*g)*b)
    wdot=(zpart(7,m)-paranl*zpart(5,m))*(wdrive+wisland+wpara+wdrift+wdriftind+wdrift0+wpara0)
if(island==1)then
if(mstepall<=2000)then
wdot=(zpart(7,m)-paranl*zpart(5,m))*wisland
endif
endif
#else
    ...
    wisland=-(kappa+kappav)*(ri*b*dapidz-g*b*dapidt)*(upara-upara0)*gqi*b
