...
subroutine chargeParticle(zpart,wppart,wtpart0,wtpart1,density,flow,&
  ...
  use equilibrium,only: lsp,spdpsi_inv,spdpsi,lst,spdtheta_inv,spdtheta,&
    bsp,xsp,spdim
  use particle_array,only:pressureipara,pressureiperp
  implicit none
...
!  kperprhoi=0.0  
...
#else
    pressureipara(0:1,ij)=0.0
    pressureiperp(0:1,ij)=0.0
#endif
  enddo
  !$acc end parallel
  ...
  if(magnetic==0)then
    ...
    !$omp& reduction(+: pressureipara,pressureiperp)
#else
! scatter particle density for electrostatic simulation
    !$omp parallel do&
    !$omp& private(m,igyro,weight,wz1,wz0,wp1,wp0,wt10,wt00,wt11,wt01,ij,dzx,dx,&
...
!integral for pressure
#ifndef _hybrid
          !$acc atomic update
          pressureipara(0,ij) = pressureipara(0,ij) + wz0*wt00*energypara
          !$acc atomic update
          pressureipara(1,ij) = pressureipara(1,ij) + wz1*wt00*energypara
          !$acc atomic update
          pressureiperp(0,ij) = pressureiperp(0,ij) + wz0*wt00*energyperp
          !$acc atomic update
          pressureiperp(1,ij) = pressureiperp(1,ij) + wz1*wt00*energyperp
#else
 !$acc atomic update
...
#ifndef _hybrid
          !$acc atomic update
          pressureipara(0,ij) = pressureipara(0,ij) + wz0*wt10*energypara
          !$acc atomic update
          pressureipara(1,ij) = pressureipara(1,ij) + wz1*wt10*energypara
          !$acc atomic update
          pressureiperp(0,ij) = pressureiperp(0,ij) + wz0*wt10*energyperp
          !$acc atomic update
          pressureiperp(1,ij) = pressureiperp(1,ij) + wz1*wt10*energyperp
#else
 !$acc atomic update
...
#ifndef _hybrid
          !$acc atomic update
          pressureipara(0,ij) = pressureipara(0,ij) + wz0*wt01*energypara
          !$acc atomic update
          pressureipara(1,ij) = pressureipara(1,ij) + wz1*wt01*energypara
          !$acc atomic update
          pressureiperp(0,ij) = pressureiperp(0,ij) + wz0*wt01*energyperp
          !$acc atomic update
          pressureiperp(1,ij) = pressureiperp(1,ij) + wz1*wt01*energyperp
#else
    !$acc atomic update
...
#ifndef _hybrid
          !$acc atomic update
          pressureipara(0,ij) = pressureipara(0,ij) + wz0*wt11*energypara
          !$acc atomic update
          pressureipara(1,ij) = pressureipara(1,ij) + wz1*wt11*energypara
          !$acc atomic update
          pressureiperp(0,ij) = pressureiperp(0,ij) + wz0*wt11*energyperp
          !$acc atomic update
          pressureiperp(1,ij) = pressureiperp(1,ij) + wz1*wt11*energyperp
#else
 !$acc atomic update
...
#else
!$acc update host(pressureipara,pressureiperp)
#endif
#endif
#endif
...
!$omp parallel do private(ij)
    do ij=1,mgrid
      dpitmppara(0:1,ij)=pressureipara(0:1,ij)
      pressureipara(0:1,ij)=0.0_wp
      dpitmpperp(0:1,ij)=pressureiperp(0:1,ij)
      pressureiperp(0:1,ij)=0.0_wp
    enddo
    call MPI_ALLREDUCE(dpitmppara,pressureipara,icount,mpi_Rsize,MPI_SUM,partd_comm,ierror)
    call MPI_ALLREDUCE(dpitmpperp,pressureiperp,icount,mpi_Rsize,MPI_SUM,partd_comm,ierror)
#endif
  endif
...
#else
pressureipara(:,igrid(i)+mtheta(i))=pressureipara(:,igrid(i)+mtheta(i))+pressureipara(:,igrid(i))
    pressureiperp(:,igrid(i)+mtheta(i))=pressureiperp(:,igrid(i)+mtheta(i))+pressureiperp(:,igrid(i))
#endif
  enddo
  ...
  do i=1,mgrid
    ...
    recvr(i,3:4)=0.0_wp
#else
sendl(i,3)=pressureipara(0,i)
    sendl(i,4)=pressureiperp(0,i)
    recvr(i,3:4)=0.0_wp
#endif
  enddo
...
#else
pressureipara(1,ii+1:ii+jt)=pressureipara(1,ii+1:ii+jt)+&
           cshift(recvr(ii+1:ii+jt,3),itran(i))
      pressureiperp(1,ii+1:ii+jt)=pressureiperp(1,ii+1:ii+jt)+&
           cshift(recvr(ii+1:ii+jt,4),itran(i))
#endif
    enddo
  else
...
#else
pressureipara(1,i)=pressureipara(1,i)+recvr(i,3)
      pressureiperp(1,i)=pressureiperp(1,i)+recvr(i,4)
#endif
    enddo
  endif
...
#else
        pressureipara(1,ij)=gyrodum*pressureipara(1,ij)
        pressureiperp(1,ij)=gyrodum*pressureiperp(1,ij)
#endif
    enddo
  enddo
...
#else
        pressureipara(1,ij)=pressureipara(1,ij)*marker(ij)
        pressureiperp(1,ij)=pressureiperp(1,ij)*marker(ij)
#endif
    enddo
  enddo
...
#else
        pressureipara(1,igrid(i))=pressureipara(1,igrid(i)+mtheta(i))
#endif
    enddo
  endif
...
#else
        pressureipara(1,igrid(i))=pressureipara(1,igrid(i)+mtheta(i))
        pressureiperp(1,igrid(i))=pressureiperp(1,igrid(i)+mtheta(i))
#endif
enddo
endif
...
#else
call smooth(pressureipara)
    call smooth(pressureiperp)
#endif
    if(nfilter>0)then
      call filter(flow)
#ifdef _hybrid
...
#else
call filter(pressureipara)
      call filter(pressureiperp)
#endif
    endif
  endif
