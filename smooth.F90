    ...
    if(nbound>99)then
      ...
      do i=0,nbound-101
        ...
        if(psi0 .gt. 0.0_wp)then
          farray(1,igrid(i):igrid(i)+mtheta(i))=farray(1,igrid(i):igrid(i)+mtheta(i))*exp(-(((i-(nbound-100)-1)/sigma)**2)/2)
          if(nboundR==0)farray(1,igrid(j):igrid(j)+mtheta(j))=farray(1,igrid(j):igrid(j)+mtheta(j))*exp(-(((i-(nbound-100)-1)/sigma)**2)/2)
        endif
      enddo
      if(nboundR>99)then !right boundary uses different number of points in boundary condition
        ...
        do i=0,nboundR-101
          j=mpsi-i
          if(psi0 .gt. 0.0_wp)then
            farray(1,igrid(j):igrid(j)+mtheta(j))=farray(1,igrid(j):igrid(j)+mtheta(j))*exp(-(((i-(nboundR-100)-1)/sigma)**2)/2)
          endif
        enddo
      endif 
    else 
