...
subroutine lorentz(species_name)
  ...
  use field_array
  use equilibrium
  use interfaces,only:collision_pitch
  implicit none
...
subroutine collision_pitch(zpart,mesht,meshn,qpart,apart,tppp,mp,pcoll,taupp,&
  ...
  real(8) v,dele,zv,ae_inv,ve,nuei,freq,upara,vperp2,pitch,b,randome(mp),rg,dp1,&
       psitmp,dpx,dp2,thetatmp,dtx,dt2,zeff,eden,etemp,ve0,ptmp,te_inv,delr,delp(mpsi)

  ae_inv=1.0/apart
  te_inv=1.0/tppp(1,1)
  delr=1.0/deltar
  delp=1.0/deltap
  ve0=rho0*sqrt(2.0*ae_inv)
...
! 1D spline in radial
     ...
     etemp=(dp1*mesht(ii-1)+(1.0-dp1)*mesht(ii))*te_inv
...
! 2D spline in (psi, theta) for B-field
     ...
     upara=zpart(4,m)*b*qpart*ae_inv
     vperp2=zpart(6,m)*zpart(6,m)*2.0*b*ae_inv
...
subroutine fokkerplanck(species_name)
  ...
  use field_array
  use equilibrium
  use interfaces,only:collision_fokkerplanck
  implicit none
  ...
  elseif(species_name=='thermal_electron')then
    call collision_fokkerplanck(zelectron,meshte,meshne,qelectron,aelectron,tepp,me,ecoll,tauee,ncyclee)  
  elseif(species_name=='thermal_ion')then
    call collision_fokkerplanck(zion,meshti,meshni,qion,aion,tipp,mi,icoll,tauii)
  endif
...
subroutine collision_fokkerplanck(zpart,mesht,meshn,qpart,apart,tppp,mp,pcoll,taupp,&
  ...
  real(wp) upara,vperp2,delu,delv2,dneop,dneot,dneoz,psitmp,thetatmp,zetatmp,dpx,dp2,dtx,dt2,b,&
       v,xv,zv,vmin,freq,phix,dphi,f,g,h,sp,sn,dp,dn,dpn,ap_inv,vp,vp0,nupp,pden,ptemp,&
       random1(mp),random2(mp),bgc(mp),delm(neop*neot*neoz),dele(neop*neot*neoz),&
...
!$omp& freq,k,phix,dphi,f,g,h,sp,sn,dp,dn,dpn,delu,delv2)
  do m=1,mp
...
! ion-ion collision frequency
     freq=nupp*pden/(zv*xv*ptemp**1.5)
