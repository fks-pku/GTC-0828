...
! Use the condition: hmesh0(w)-hmesh0(ires)=2*alpha to find the island boundaries, Eq(17)
  do i=0,mpsi
     ...
     dhmesh0(i)=abs(hmesh0(i)-2.0*-0.0001)
  enddo
...
! find the island boundaries in psi
  ...
  enddo
if(mype==0)then
write(gtcout,*)'ires=',ires,'mtheta at iflux=',mtheta(ires),'wi=',wi,'wo=',wo
write(gtcout,*)'island width=',(sprgpsi(psimesh(wo))-sprgpsi(psimesh(wi)))/sprgpsi(ped)
write(gtcout,*)'dianostic flux surface parameters:'
write(gtcout,*)'r/a=',sprgpsi(psimesh(ires))/sprgpsi(ped),'R0/L_Ti=',(eqmeshti(ires-1)-eqmeshti(ires+1))/(sprgpsi(psimesh(ires-1))-sprgpsi(psimesh(ires+1)))/eqmeshti(ires),'R0/L_ni=',(eqmeshni(ires-1)-eqmeshni(ires+1))/(sprgpsi(psimesh(ires-1))-sprgpsi(psimesh(ires+1)))/eqmeshni(ires),'magnetic shear=',(alog(qmesh(ires-1))-alog(qmesh(ires+1)))/(alog(sprgpsi(psimesh(ires-1)))-alog(sprgpsi(psimesh(ires+1))))

open(12345678,file='eqprofile.out',status='replace')
do i=0,mpsi
write(12345678,'(3f12.6)')sprgpsi(psimesh(i)),psimesh(i),qmesh(i)
enddo
close(12345678)
...
!  enddo
   ...
   do n=1,l
     do i=0,mpsi
     alpnm(n,i)=-0.0001 !example profile of the island
...
open(654321,file='alphaIs.out',status='replace')
if(mype==0)then
do i=0,mpsi
  do j=1,mtheta(i)
     ij=igrid(i)+j
write(654321,'(4f12.6)')sprgpsi(psimesh(i)),j*2*3.1415926/mtheta(i),alphaIs(1,ij),hmesh0(i)-alphaIs(1,ij)
enddo
enddo
endif
...
!=============================================================================
  ...
  integer,parameter :: n_integers=52+num_modes+num_modes, n_reals=25
...
! Pack all parameters in integer_params & real_params
  if(mype==0)then
    ...
    integer_params(53:52+num_modes)=n_modes(1:num_modes)
    integer_params(53+num_modes:52+num_modes+num_modes)=m_modes(1:num_modes)
...
!   Unpack all parameters
  if(mype/=0)then
    ...
    n_modes(1:num_modes)=integer_params(53:52+num_modes)
    m_modes(1:num_modes)=integer_params(53+num_modes:52+num_modes+num_modes)
...
!=============================================================================
  ...
  namelist /domain_parameters/ toroidaln
  !default is no partial torus
  toroidaln=1
  inquire(file='gtc.in',exist=file_exist)
  if (file_exist) then
      open(55,file='gtc.in',status='old')
      read(55,nml=domain_parameters)
      close(55)
  else
      write(*,*)'Defaulted to full torus !!!'
  endif
...
! Domain decomposition in toroidal direction.
  if(toroidaln<1.0_wp)then
     write(gtcout,*)'Please check gtc.in for toroidaln (must be equal to or larger than 1)!!!',&
...
! open standard output file, record program starting time
  if(mype==0)then
     ...
     write(gtcout,*)"GTC Version V2016_0108_update0828"
...
subroutine gaussbound(y)
  ...
  do i=0,nbound-101
    y(i)=y(i)*exp(-(((i-(nbound-100)-1)/sigma)**2)/2)
    if(nboundR==0)y(mpsi-i)=y(mpsi-i)*exp(-(((i-(nbound-100)-1)/sigma)**2)/2)
  enddo
...
subroutine gaussboundR(y)
  ...
  do i=0,nboundR-101
    y(mpsi-i)=y(mpsi-i)*exp(-(((i-(nboundR-100)-1)/sigma)**2)/2)
  enddo
