subroutine eqdata
  ...
  integer i,icount,ierror,isp,iformat,lemax,lrmax,ndim,nfp,sgn,lsp_iflux
  ...
  if(numereq==1)then
     if(mype==0)then
        ...
        if (iformat==1) then
           ...
           read(isp,101) psiw,ped,rmaj !psiw: poloidal flux at wall; ped - at last closed surface
...
!!!FOR David EP case: change psi from negative to positive
           psiw=abs(psiw)
           ped=abs(ped)
           lst=lst+1 !in ORBIT spdtheta=2pi/lst; needs to raise lst by one after reading spdata.
        elseif (iformat==3) then ! non-axisymmetric case
...
! allocate memory for equilibrium spline array
  ...
  if(mype==0)then
     if(numereq==1)then ! use EFIT & TRANSP data
...
! read data and constructing splines for non-axisymmetric equilibrium
  if (numereq==1.and.iformat==3)then
...
! poloidal spline fit of sin and cos functions
  ...
  if (iformat/=3) then
! radial and poloildal spline fit of B field and (X,Z) coordinates fo axisymmetric equil.
...
! modification for various fielddir in analytic equilibrium
  if (numereq==0) then
...
! ropp- toroidal angular velocty, erpp=-dphi_0dp
  if(numereq==1)then ! numerical equilibrium unit conversion from TRANSP TO GTC
...
! assume Carbon impurity
  ...
  if(mype==0)then
     ...
     write(gtcout,*)"nue_eff=",qpsi(1,1)*sqrt(aelectron/aion)/(rho0*tauei),&
          "nui_eff=",qpsi(1,1)/(sqrt(tipp(1,1))*tauii)
     call FLUSH(gtcout)
  endif
...
subroutine spdata(iformat,isp)
  use global_parameters
  use equilibrium
  use spline_normalization
  implicit none

  integer i,krip,nrip,iformat,isp,ipsi
